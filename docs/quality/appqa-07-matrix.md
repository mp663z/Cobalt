...
is from the sweep at `ac92756` (44/44 passed, 44/44 reopened offline).
...
| --- | --- | --- | --- | --- | --- |
| arxiv | UNVERIFIED | none evidenced | gap - keeping a paper needs a fetched paper; no offline write path | yes - apps/arxiv/drive.kobo | yes |
| audiobook | UNVERIFIED | none evidenced | gap - the library records only finished books; composing needs provider secrets (exa/elevenlabs), no draft persists on failure | yes - examples/audiobook/drive.kobo | yes |
| backgammon | UNVERIFIED | none evidenced | yes - journey wrote backgammon-autosave-v4 | yes - apps/backgammon/drive.kobo | yes |
| brief | UNVERIFIED | none evidenced | gap - keeping an article needs a fetched digest; no offline write path | yes - examples/brief/drive.kobo | yes |
| calibre-web | UNVERIFIED | none evidenced | gap - settings save only after a verified server fetch | yes - apps/calibre-web/drive.kobo | yes |
...
| fanshelf | UNVERIFIED | FANSHELF_DEMO synthetic library exercised by the route | yes - journey wrote tags.v1 | yes - apps/fanshelf/drive.kobo | yes |
| fieldbook | UNVERIFIED | none evidenced | yes - journey wrote sightings | yes - apps/fieldbook/drive.kobo | yes |
| flashcards | UNVERIFIED | original demo bundle staged like a host import, reviewed by the route | yes - journey wrote cobalt-review-log.ndjson | yes - apps/flashcards/drive.kobo | yes |
...
| hn | UNVERIFIED | none evidenced | gap - read marks need a fetched story list; no offline write path | yes - examples/hn/drive.txt | yes |
| homepanel | UNVERIFIED | none evidenced | gap - requires an https Home Assistant URL; the simulator has no TLS fixture | yes - apps/homepanel/drive.kobo | yes |
...
| lichess | UNVERIFIED | none evidenced | gap - live play needs an account token; the offline computer game does not persist | yes - apps/lichess/drive.kobo | yes |
...
| panels | UNVERIFIED | none evidenced | gap - panel content comes from the paired host; no offline write path | yes - apps/panels/drive.kobo | yes |
| paperterm | UNVERIFIED | none evidenced | gap - pairing needs a reachable terminal host; no offline write path | yes - apps/paperterm/drive.kobo | yes |
| parlor | UNVERIFIED | none evidenced | yes - journey wrote parlor-autosave-v1 | yes - apps/parlor/drive.kobo | yes |
| parser | UNVERIFIED | synthetic demo story staged like a device push, played by the route | yes - journey wrote save-1-260901-0000-auto | yes - apps/parser/drive.kobo | yes |
| post | UNVERIFIED | none evidenced | yes - journey wrote gateway | yes - apps/post/drive.kobo | yes |
| pubquiz | UNVERIFIED | none evidenced | yes - journey wrote pubquiz-state | yes - apps/pubquiz/drive.kobo | yes |
| readlater | UNVERIFIED | none evidenced | yes - journey wrote config | yes - apps/readlater/drive.kobo | yes |
| rss | UNVERIFIED | none evidenced | gap - every save path (subscribe, remove, retry) needs a fetched feed first | yes - examples/rss/drive.kobo | yes |
| rss-miniflux | UNVERIFIED | none evidenced | gap - server-first client; no offline write path | yes - apps/rss-miniflux/drive.kobo | yes |
| sidekick | UNVERIFIED | none evidenced | gap - pairing needs the sidekick handshake; no offline write path | yes - examples/sidekick/drive.txt | yes |
...
- own-data gaps (manifest declares user data, no own-data evidence):
  arxiv, audiobook, brief, calibre-web, gutenbird, hn, homepanel, lichess, panels, paperterm, rss, rss-miniflux, sidekick, zotero-reader.
- sample: no sample-path evidence for 38 apps;
